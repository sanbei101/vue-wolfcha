 ...
 *
 * 头像配置规则 (与 Next.js 版 wolfcha 完全一致):
...
const NON_FEMALE_HAIR: string[] = ALL_HAIR_VARIANTS.filter(
  (v) => !FEMALE_ONLY_HAIR.includes(v as (typeof FEMALE_ONLY_HAIR)[number]),
);
...
// 禁止使用的嘴型
const FORBIDDEN_LIPS: readonly string[] = ["variant01", "variant02", "variant05"] as const;
...
const IDLE_LIPS: string[] = ALL_LIPS_VARIANTS.filter(
  (v) =>
    !FORBIDDEN_LIPS.includes(v as (typeof FORBIDDEN_LIPS)[number]) &&
    !TALKING_LIPS.includes(v as (typeof TALKING_LIPS)[number]),
);
...
const DAY_EYES: string[] = ALL_EYES_VARIANTS.filter(
  (v) => !FORBIDDEN_EYES.includes(v as (typeof FORBIDDEN_EYES)[number]),
);
...
export function getAvatarBgColor(seed: string): string {
  return AVATAR_BG_COLORS[hashString(seed) % AVATAR_BG_COLORS.length] ?? "e8d5c4";
}

/**
 * 根据性别获取可用的发型列表
 */
export function getHairVariantsForGender(gender: string): string[] {
  if (gender === "female") {
 ...
 * 根据 seed 和性别获取稳定的发型
 */
export function getHairForSeed(seed: string, gender: string): string {
  const variants = getHairVariantsForGender(gender);
  return variants[hashString(seed) % variants.length] ?? "variant03";
}
...
export function getDayEyesForSeed(seed: string): string {
  return DAY_EYES[hashString(seed) % DAY_EYES.length] ?? "variant01";
}
...
export function getIdleLipsForSeed(seed: string): string {
  return IDLE_LIPS[hashString(seed) % IDLE_LIPS.length] ?? "variant03";
}
...
export function buildAvatarUrl(options: AvatarUrlOptions): string {
  const { seed, gender, eyes, lips, hair, scale = 100, translateY = 0, backgroundColor } = options;
...
export function getPlayerAvatarUrl(player: Player): string {
  const seed = player.avatarSeed ?? player.playerId;
  return buildSimpleAvatarUrl(seed, {
    gender: (player as { gender?: AvatarGender }).gender,
  });
}
